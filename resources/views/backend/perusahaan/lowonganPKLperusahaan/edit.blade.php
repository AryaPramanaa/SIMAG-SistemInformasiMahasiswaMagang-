    ...
    <div class="min-h-screen py-8 px-4 md:px-8">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                <form action="{{ route('perusahaan.lowonganPKL.update', $lowonganPKL->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6">
                        <!-- Divisi -->
                        ...
                        <div>
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="4" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('deskripsi', $lowonganPKL->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="syarat" id="syarat" rows="4" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('syarat', $lowonganPKL->syarat) }}</textarea>
                            @error('syarat')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
